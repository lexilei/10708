...
\]
即使图不是 tree 也这么算。这叫 \textbf{Bethe entropy approximation}。加上 energy 项就是 \textbf{Bethe free energy}，跟 belief propagation 算法有直接联系。
...
\begin{tabular}{l|l|l}
  ...
  有 loop & Bethe approximation（硬用 tree 公式） & 近似（上界） \\
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{E-step}：固定 $\theta$，找最优的 $q(z)$ 来逼近 posterior $p(z|x)$（这就是 VI）
  \item \textbf{M-step}：固定 $q$，更新 $\theta$ 来 maximize ELBO
\end{itemize}

就是对 ELBO 做 coordinate ascent：一步优化 $q$，一步优化 $\theta$。
...
EM 的问题：E-step 要对每个数据点单独做 VI，太慢。

VAE 的 idea：用一个神经网络 $q_\phi(z|x)$ 直接把 $x$ 映射到 $q$（叫 \textbf{amortized inference} --- 一个网络处理所有数据点，不用每个单独优化）。然后 $\theta$ 和 $\phi$ 一起用 gradient descent 训练。

难点：ELBO 里有 $\mathbb{E}_{z \sim q}[\cdot]$，怎么对采样求导？$\to$ \textbf{Reparameterization trick}：把 $z \sim q_\phi$ 写成 $z = \mu_\phi + \sigma_\phi \cdot \epsilon$，$\epsilon \sim \mathcal{N}(0,1)$，这样 gradient 就能穿过去了。
...
MLE 需要 $\nabla_\theta \log p_\theta(x)$，里面有 $\nabla \log Z$ 很烦。

Score matching 的 idea：不匹配分布本身，匹配它的 \textbf{score function} $\nabla_x \log p(x)$。因为 $\nabla_x \log \frac{e^{-E(x)}}{Z} = -\nabla_x E(x)$，$Z$ 是常数，求导直接消掉了。
